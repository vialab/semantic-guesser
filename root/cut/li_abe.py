...
def compute_ddl(cut, sample_size, estimator = None):
    ...
    """
    prob = lambda freq, s: estimator.probability(freq) if estimator else pc(freq, sample_size)
    return ddl([(c[1], pn(prob(c[1], sample_size), c[2])) for c in cut])
...
def compute_dl(cut, sample_size, estimator=None):
    ...
    # print compute_ddl(cut, sample_size, estimator)
    # print compute_pdl(cut, sample_size)
