...
class SentiWordNetCorpusReader:
    ...
    def senti_synset(self, *vals):        
        if tuple(vals) in self.db:
            ...
            if (pos, offset) in self.db:
                ...
                return SentiSynset(pos_score, neg_score, synset, offset)
            else:
                return None
    ...
    def all_senti_synsets(self):
        for key, fields in self.db.iteritems():
            ...
            synset = wn._synset_from_pos_and_offset(pos, offset)
            yield SentiSynset(pos_score, neg_score, synset, offset)

######################################################################
            
class SentiSynset:
    def __init__(self, pos_score, neg_score, synset, offset):
